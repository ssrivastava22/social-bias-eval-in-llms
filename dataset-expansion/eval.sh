...
OPENAI_API_KEY=$API_KEY1 python3 dataset-expansion/eval_model_on_extended_data.py --input "${CHUNK_PREFIX}${SUFFIXES[0]}" --output "${CHUNK_PREFIX}0.jsonl" &
OPENAI_API_KEY=$API_KEY2 python3 dataset-expansion/eval_model_on_extended_data.py --input "${CHUNK_PREFIX}${SUFFIXES[1]}" --output "${CHUNK_PREFIX}1.jsonl" &
OPENAI_API_KEY=$API_KEY3 python3 dataset-expansion/eval_model_on_extended_data.py --input "${CHUNK_PREFIX}${SUFFIXES[2]}" --output "${CHUNK_PREFIX}2.jsonl" &
wait

# Merge the results
cat "${CHUNK_PREFIX}"[012].jsonl > "${OUTPUT_PREFIX}_evaluated_results.jsonl"

echo "Merged results written to ${OUTPUT_PREFIX}_evaluated_results.jsonl"
