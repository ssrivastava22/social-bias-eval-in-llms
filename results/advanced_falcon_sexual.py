...
def extract_sexual_orientation_groups(context):
    groups = []
    group_patterns = [
    ...
    for group in group_patterns:
        if group in context:
            groups.append(group.replace(" man", "").replace(" woman", "").replace(" one", "").replace(" friend", "").capitalize())
    return groups

def sexual_orientation_error_analysis(results_path):
    ...
    for entry in results:
        ...
        groups_in_context = extract_sexual_orientation_groups(context_text)
...
sexual_orientation_error_analysis("falcon_bbq_results_sexual_orientation.json")